...
def quit():
	...
	conn.close()
	s.close()
	# The easiest way I found to avoid all errors with program client quitting was to simply restat the server.
	# This avoids any errors associated with socket connections etc.
	os.execl(sys.executable, sys.executable, *sys.argv)

while True:
	# Enter into a while loop to recieve commands from client
	# This is done with a while loop for the reason explained in the client
	...
	if data == "UPLD":
		...
		quit()
	# Reset the data to loop (not really necessary, done for completeness)
	data = None
